...
@pytest.mark.parametrize(
    ...
    [
        ('Any value different from int', 0),
        (None, 0),
        (0, 0),
        (1, 1),
        (2, 2),
...
async def test_instatiation_params(priority, expected_priority) -> None:
    ...
    assert sut.vars == {}
    assert sut.status == 'filled'
    with pytest.raises(NotImplementedError):
        assert sut.is_scheduled == False


@pytest.mark.parametrize(
    ['status', 'expected_is_sent'],
    [('queued', True), ('sent', True), ('filled', False)],
)
async def test_ensure_is_sent_property_follows_status_property(
    status, expected_is_sent
) -> None:
    sut = BaseNotification(title=title, content=content, priority=1)
    sut.status = status
    assert sut.is_sent == expected_is_sent
